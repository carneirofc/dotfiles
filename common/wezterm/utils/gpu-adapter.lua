...
---Backend options available for each platform.
...
function GpuAdapters:init()
   ...
   for _, adapter in ipairs(ENUMERATED_GPUS) do
      ...
      local device_type_score = self.device_types[adapter.device_type]

      -- wgpu may report a backend this platform's table does not score (e.g. `BrowserWebGpu`)
      if backend_score and device_type_score then
...
---
---NOTE: on a hybrid-graphics laptop this prefers the discrete GPU, which costs battery for
---little terminal gain. Use `GpuAdapters:pick_manual('Vulkan', 'IntegratedGpu')` to pin the iGPU.
---@return GpuInfo|nil
